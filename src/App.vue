...
const fetchWeather = async () => {
  ...
  try {
    ...
    if (geoData.results.length === 1) {
      city.value = geoData.results[0].name
      return selectCity(geoData.results[0])
    }
      ...
      <template v-if="showComponents">
        ...
        <section class="daydetails">
          <div v-for="(day, index) in dailyForecast" :key="index" class="column daycolumn">
            ...
            <p class="small">min {{ Math.round(day.min) }}°</p>
            <p class="small">max {{ Math.round(day.max) }}°</p>
            <p class="small">🌧 {{ day.proba_max || 0 }}%</p>
          </div>
        </section>
